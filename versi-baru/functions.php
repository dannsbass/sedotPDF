...
function daftarkanPdf($url, $file_id, $title){
  if(!Dannsheet::findRowByValue($url, 'SedotPDFBot!A1:A')) Dannsheet::appendRow([[$url, $file_id, $title]], 'SedotPDFBot');
}
...
function masukkanDataBaru($json, $document_url){
  ...
  if ($res->result->document->mime_type == 'application/pdf'){
    ...
    Dannsheet::appendRow([[$document_url, $file_id, $title]], 'SedotPDFBot');
...
function getSavedLinks(){
  ...
  if($A_values = Dannsheet::getValues('SedotPDFBot!A1:B')){
